...
export function LiquidationMap({
  ...
  const fetcher = (mt: string) =>
    api.getVergexCostLiquidationHeatmap({
      marketType: mt,
      symbol,
      chain: 'mainnet',
      liqBand: '15',
  ...
  const primary = useSWR(
    symbol && !demo ? ['heatmap', marketType, symbol] : null,
    () => fetcher(marketType),
    opts
  )
  ...
  const needAlt =
    !demo && !primaryHasBins && !primary.isLoading && primary.data !== undefined
  const alt = useSWR(
    needAlt && symbol ? ['heatmap', altMt, symbol] : null,
    () => fetcher(altMt),
    opts
  )
  ...
  const demoBase = useMemo(() => {
    ...
    return {
      ...
      mark,
      bins,
      costAddrs: Math.round(lmRnd(22000, 31000)),
      liqAddrs: Math.round(lmRnd(16000, 23000)),
    }
  }, [demo, symbol])
  ...
  const demoData = useMemo(() => {
    ...
    const f = demoFrame
    const w = (v: number, amp: number, ph: number) =>
      v * (1 + amp * Math.sin(f * 0.5 + ph))
    const bins: VergexHeatmapBin[] = demoBase.bins.map(
      (b, i) =>
        ({
          px: b.px,
    ...
    return {
      data: {
        bins,
        markPrice: demoBase.mark,
        costAddrs: demoBase.costAddrs,
        liqAddrs: demoBase.liqAddrs,
        market: { symbol: demoBase.base },
      },
    }
  }, [demoBase, demoFrame])

  const data = demo ? demoData : primaryHasBins ? primary.data : alt.data
  const isLoading = demo
    ? false
    : primary.isLoading || (needAlt && alt.isLoading)
  const error = demo
    ? undefined
    : primaryHasBins
      ? undefined
      : alt.error || primary.error
  ...
  const view = useMemo(() => {
    ...
    const rows: Row[] = raw
      ...
      .filter(
        (r) =>
          r.px > 0 && (r.longCost || r.shortCost || r.longLiq || r.shortLiq)
      )
      .sort((a, b) => b.px - a.px)
    ...
    return {
      rows,
      maxSide,
      mark: stale ? 0 : (d?.markPrice ?? 0),
      costAddrs: stale ? 0 : (d?.costAddrs ?? 0),
      liqAddrs: stale ? 0 : (d?.liqAddrs ?? 0),
      totals,
      dispSymbol: stale ? symbol : d?.market?.symbol || symbol,
    }
  }, [data, symbol])
    ...
    <div style={{ fontFamily: 'var(--tm-mono)' }}>
      ...
      >
        ...
        <span
          ...
          style={{
            marginLeft: 'auto',
            color: view.rows.length ? 'var(--tm-up)' : 'var(--tm-muted)',
          }}
        >
      ...
      >
        {hv ? (
          <span>
            <b>{fmtPx(hv.px)}</b> · Cost line{' '}
            <span style={{ color: C_LONG_COST }}>{fmtUsd(hv.longCost)}</span>/
            <span style={{ color: C_SHORT_COST }}>{fmtUsd(hv.shortCost)}</span>
            {' · '}liq{' '}
            <span style={{ color: C_LONG_LIQ }}>{fmtUsd(hv.longLiq)}</span>/
            <span style={{ color: C_SHORT_LIQ }}>{fmtUsd(hv.shortLiq)}</span>
          </span>
        ) : (
          <span className="tm-sc">
            标记价{' '}
            <b style={{ color: 'var(--tm-red)' }}>
              {view.mark ? fmtPx(view.mark) : '—'}
            </b>{' '}
            · {view.costAddrs.toLocaleString()} 仓位 /{' '}
            {view.liqAddrs.toLocaleString()} 清算位
          </span>
        )}
      </div>
      ...
      {error && !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>
          {view.dispSymbol} 暂无成本/清算图（加密货币/主 DEX 市场无此图）。
        </div>
      ) : !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>
          正在加载成本/清算图…
        ...
        <div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    height: rowH,
                    background:
                      hover === i ? 'rgba(26,24,19,0.05)' : 'transparent',
                ...
                >
                  <span
                    style={{
                      ...
                      textAlign: 'right',
                      paddingRight: 6,
                      color: isMark ? 'var(--tm-red)' : 'var(--tm-muted)',
                      fontWeight: isMark ? 700 : 400,
                    }}
                  >
                  ...
                  >
                    <div
                      style={{
                        width: `${slW}%`,
                        background: C_SHORT_LIQ,
                        opacity: 0.85,
                      }}
                    />
                    <div
                      style={{ width: `${scW}%`, background: C_SHORT_COST }}
                    />
                  </div>
                  ...
                  >
                    <div
                      style={{ width: `${lcW}%`, background: C_LONG_COST }}
                    />
                    <div
                      style={{
                        width: `${llW}%`,
                        background: C_LONG_LIQ,
                        opacity: 0.85,
                      }}
                    />
                  </div>
          ...
          >
            <span>
              成本线{' '}
              <span style={{ color: C_LONG_COST }}>
                {fmtUsd(view.totals.lc)}
              </span>
              /
              <span style={{ color: C_SHORT_COST }}>
                {fmtUsd(view.totals.sc)}
              </span>
            </span>
            <span>
              清算{' '}
              <span style={{ color: C_LONG_LIQ }}>
                {fmtUsd(view.totals.ll)}
              </span>
              /
              <span style={{ color: C_SHORT_LIQ }}>
                {fmtUsd(view.totals.sl)}
              </span>
            </span>
          </div>
        </div>
