...
function Row({ px, sz, cum, maxCum, side, h, mark }: RowProps) {
  ...
  // short by the 60fps re-renders. Adjusting state during render is the React
  // sanctioned derived-state pattern (refs must not be touched during render).
  const [flash, setFlash] = useState({ prev: sz, dir: '' })
  if (sz !== flash.prev) {
    setFlash({ prev: sz, dir: sz > flash.prev ? 'ob-up' : 'ob-dn' })
  }
